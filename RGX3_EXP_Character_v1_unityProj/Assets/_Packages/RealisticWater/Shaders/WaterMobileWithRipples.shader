...
struct v2f {
	...
	fixed4 reflectionDir : TEXCOORD3;
};
...
v2f vert (appdata_t v)
{
	...
	//--------------------Gerstner waves-------------
	fixed2 vtxForAni = posWorld /_WaveScale; 			
	...
	float2 texDisp = 0;
	#if ripples_on
	fixed2 tileableUv = mul(_projectiveMatrWaves,v.vertex).xz;;
	texDisp = tex2Dlod(_WaterDisplacementTexture, float4(tileableUv,0,0)).rg;
	float displ = texDisp.r - texDisp.g;
	texDisp.g = -texDisp.g;
	v.vertex.y += displ;
	o.reflectionDir.w = clamp(displ  +  displ * displ*2, 0, 0.3) ;
	#endif
...
{	
	...
	offset += i.reflectionDir.w;
	
	#if ripples_on
	offset += i.reflectionDir.w*offset;
	#endif
	...
	#endif
	#if cubeMap_on
	reflection =  texCUBE(_Cube, i.reflectionDir.xyz) * _ReflectionColor.rgb;
	#endif
	fixed3 grab = tex2Dproj(_GrabTextureMobile, coord).rgb;
	fixed3 col = grab * _Color.rgb + reflection;
	return fixed4(col, 1);
}
ENDCG
		}
